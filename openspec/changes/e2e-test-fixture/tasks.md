...
- [ ] 1.1 Create `tests/e2e/main.tf` — provider block (`owner = var.github_org`), module call to `../../` with `config_path`, `repository_partitions = ["partitioned"]`, `webhook_secrets`, `membership_management_enabled`
- [ ] 1.2 Create `tests/e2e/variables.tf` — `github_org` (string), `test_user` (string, default `""`), `webhook_secret` (string, sensitive, default `"e2e-test-secret"`), `membership_management_enabled` (bool, default `false`)
...
- [ ] 1.4 Create `tests/e2e/.gitignore` — ignore `.terraform/`, `.terraform.lock.hcl`, `tfplan`, `*.tfstate`, `*.tfstate.backup`, `terraform.tfvars`
- [ ] 1.5 Verify: `cd tests/e2e && terraform init && TF_VAR_github_org=placeholder terraform validate` exits 0
...
- [ ] 3.1 Create `tests/e2e/config/group/test-groups.yml` with four groups:
...
- [ ] 4.1 Create `tests/e2e/config/webhook/test-webhooks.yml` with two entries:
...
- [ ] 5.1 Create `tests/e2e/config/ruleset/test-rulesets.yml` with:
...
- [ ] 6.1 Create `tests/e2e/config/branch-protection/test-branch-protections.yml` with two entries:
...
- [ ] 7.1 Create `tests/e2e/config/team/test-teams.yml` with a 3-level hierarchy:
...
- [ ] 8.1 Create `tests/e2e/config/membership/test-members.yml` — comment-only file with the placeholder `# YOUR_GITHUB_USERNAME: member` and a warning comment that membership is disabled by default

## 9. Repositories

- [ ] 9.1 Create `tests/e2e/config/repository/test-repos.yml` with these repos:
...
- [ ] 9.2 Create `tests/e2e/config/repository/partitioned/partitioned-repos.yml` — single repo `e2e-partitioned-repo`: groups `[base, oss-e2e]`, topics: [partitioned]

## 10. Makefile and tfvars

- [ ] 10.1 Create `tests/e2e/Makefile` with targets: `help`, `init`, `validate`, `plan`, `plan-show`, `apply`, `verify`, `destroy`, `clean`; `verify` target does `terraform output -json > /tmp/e2e_outputs.json && python3 ../verify_e2e.py /tmp/e2e_outputs.json`
- [ ] 10.2 Create `tests/e2e/terraform.tfvars.example` with `github_org`, `test_user`, `webhook_secret`, `membership_management_enabled` with comments explaining each
- [ ] 10.3 Append `e2e-init`, `e2e-validate`, `e2e-plan`, `e2e-apply`, `e2e-verify`, `e2e-destroy` delegating targets to root `Makefile` using `$(MAKE) -C tests/e2e <target>`

## 11. Verification script

- [ ] 11.1 Create `tests/verify_e2e.py` with: CLI argument for outputs JSON file path, `GITHUB_TOKEN` env var check, `CheckResult` dataclass tracking passed/failed with `ok()`, `fail()`, `assert_eq()`, `assert_contains()` helpers
- [ ] 11.2 Implement `verify_repositories()` — for each repo in outputs, call `gh_org.get_repo(name)` and assert `visibility` matches
- [ ] 11.3 Implement `verify_teams()` — for each team slug in outputs, call `gh_org.get_team_by_slug(slug)` and assert it exists
- [ ] 11.4 Implement `verify_subscription_warnings()` — assert non-null on free tier and `e2e-internal-private` is in `repos` list
- [ ] 11.5 Implement `verify_skipped_org_rulesets()` — assert non-null on free/pro tier, null on team/enterprise
- [ ] 11.6 Implement `verify_org_webhooks()` — assert `e2e-org-webhook` key present in output
- [ ] 11.7 Implement `verify_no_duplicate_warnings()` — assert `duplicate_key_warnings` output is null
- [ ] 11.8 Implement `verify_partitioned_repo_loaded()` — assert `e2e-partitioned-repo` in repositories output
- [ ] 11.9 Wire all checks into `main()` with section headers; print summary "N passed, N failed"; exit 0 on all pass, non-zero on any failure
- [ ] 11.10 `chmod +x tests/verify_e2e.py` and verify: `python3 -m py_compile tests/verify_e2e.py`
...
- [ ] 13.1 Validate all YAML files parse cleanly: `for f in $(find tests/e2e/config -name '*.yml'); do python3 -c "import yaml; yaml.safe_load(open('$f'))" || echo "FAILED: $f"; done`
- [ ] 13.2 Run `terraform validate` in `tests/e2e/`: `cd tests/e2e && terraform init -backend=false && TF_VAR_github_org=placeholder terraform validate`
- [ ] 13.3 Run `python3 -m py_compile tests/verify_e2e.py`
