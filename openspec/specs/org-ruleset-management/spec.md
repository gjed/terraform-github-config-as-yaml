# org-ruleset-management Specification

## Purpose

Define the requirements for organization-level rulesets that apply rules across multiple
repositories based on repository name patterns.
...
### Requirement: Repository Rulesets Scope Separation

Repository rulesets are limited to definitions with `scope: repository` or no
...
The system SHALL skip organization-level rulesets on `free` and `pro` plans, and
